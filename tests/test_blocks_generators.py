...
class TestBlocksGenerator(unittest.TestCase):
    ...
    def test_lambdafold(self):
        ...
        reversed_index_alice = lambdafold_alice.build_inverted_index(records_alice, 0, random_state=0)

        # party Bob
        records_bob = [[3, "Joyce", "Wang"],
                       [4, "Lindsay", "Lin"]]
        lambda_bob = PPRLIndexLambdaFold(config)
        reversed_index_bob = lambda_bob.build_inverted_index(records_bob, 0, random_state=0)
        ...
        assert list(filtered_alice.values()) == [[1]]
        assert list(filtered_bob.values()) == [[3]]
